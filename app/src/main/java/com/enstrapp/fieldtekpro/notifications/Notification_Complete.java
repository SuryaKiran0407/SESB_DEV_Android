...
public class Notification_Complete {
    ...
    public static String Get_NOCO_Data(Context activity, String notification_id) {
        try {
            ...
            FieldTekPro_SharedPref = activity.getSharedPreferences("FieldTekPro_SharedPreferences",
                    MODE_PRIVATE);
            ...
            Cursor cursor = App_db.rawQuery("select * from Get_SYNC_MAP_DATA where Zdoctype = ?" +
                    " and Zactivity = ? and Endpoint = ?", new String[]{"Q", "S", webservice_type});
            ...
            Retrofit retrofit = new Retrofit.Builder().addConverterFactory(GsonConverterFactory
                    .create()).baseUrl(URL).client(client).build();
