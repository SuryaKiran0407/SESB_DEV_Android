...
public class Notifications_WorkCenter_Select_Activity extends AppCompatActivity implements View.OnClickListener
{
                ...
                if (cursor != null && cursor.getCount() > 0)
                {
                    if (cursor.moveToFirst())
                    {
                        do
                        {
                            ...
                            {
                                Type_Object nto = new Type_Object(cursor.getString(7), cursor.getString(8), true);
                                type_list.add(nto);
                            }
                            else
                            {
                                Type_Object nto = new Type_Object(cursor.getString(7), cursor.getString(8), false);
                                type_list.add(nto);
                            }
                        }
    ...
    SearchView.OnQueryTextListener listener = new SearchView.OnQueryTextListener()
    {
        ...
        public boolean onQueryTextChange(String query)
        {
            ...
            if(filteredList.size() > 0)
            {
                RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(Notifications_WorkCenter_Select_Activity.this);
                list_recycleview.setLayoutManager(layoutManager);
                adapter = new TYPE_ADAPTER(Notifications_WorkCenter_Select_Activity.this, filteredList);
                list_recycleview.setAdapter(adapter);
    ...
    public void onClick(View v)
    {
        ...
        else if (v == submit_button)
        {
            ...
            {
                error_dialog.show_error_dialog(Notifications_WorkCenter_Select_Activity.this,"Please Select Atleast One Workcenter");
            }
        }
    }
