        ...
        if (bundle != null)
        {
            ...
            if(activity_parcablearray.size() > 0)
            {
                for(int i = 0; i < activity_parcablearray.size(); i++)
                {
                    ...
                    try
                    {
                        Cursor cursor = App_db.rawQuery("select * from EtNotifActivity_CustomInfo where Qmnum = ? and ActvKey = ?", new String[]{nhp.getQmnum(), ActvKey});
            ...
            holder.data_layout.setOnClickListener(new View.OnClickListener()
            {
                ...
                public void onClick(View v)
                {
                    ...
                    if (causecode_list.size() > 0)
                    {
                        ...
                        intent.putExtra("selected_activity_custom_info_arraylist", nto.getSelected_activity_custom_info_arraylist());
                        startActivityForResult(intent, add_activity_type);
                    }
                }
            });
