...
public class Orders_StorageLocation_Filter_Activity extends AppCompatActivity implements View.OnClickListener
{
    ...
    private class Get_Notification_Types extends AsyncTask<Void, Integer, Void>
    {
        ...
        {
            super.onPreExecute();
            custom_progress_dialog.show_progress_dialog(Orders_StorageLocation_Filter_Activity.this,getResources().getString(R.string.loading));
        ...
        protected Void doInBackground(Void... params)
        {
            try
            {
                Cursor cursor = FieldTekPro_db.rawQuery("select * from GET_SLOC where Werks = ?", new String[]{iwerk});
                if (cursor != null && cursor.getCount() > 0)
                {
                    if (cursor.moveToFirst())
                    {
                        do
                        {
                            ...
                            {
                                Notification_Type_Object nto = new Notification_Type_Object(cursor.getString(3), cursor.getString(4), true);
                                notification_type_list.add(nto);
                            }
                            else
                            {
                                Notification_Type_Object nto = new Notification_Type_Object(cursor.getString(3), cursor.getString(4), false);
                                notification_type_list.add(nto);
                            }
                        }
        ...
        protected void onPostExecute(Void result)
        {
            ...
            {
                title_textview.setText(getResources().getString(R.string.storage_location)+" ("+notification_type_list.size()+")");
                adapter = new NOTIFICATION_TYPE_ADAPTER(Orders_StorageLocation_Filter_Activity.this, notification_type_list);
                list_recycleview.setHasFixedSize(true);
                RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(Orders_StorageLocation_Filter_Activity.this);
    ...
    SearchView.OnQueryTextListener listener = new SearchView.OnQueryTextListener()
    {
        ...
        public boolean onQueryTextChange(String query)
        {
            ...
            if(filteredList.size() > 0)
            {
                RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(Orders_StorageLocation_Filter_Activity.this);
                list_recycleview.setLayoutManager(layoutManager);
                adapter = new NOTIFICATION_TYPE_ADAPTER(Orders_StorageLocation_Filter_Activity.this, filteredList);
                list_recycleview.setAdapter(adapter);
    ...
    public void onClick(View v)
    {
        ...
        else if (v == submit_button)
        {
            ...
            {
                error_dialog.show_error_dialog(Orders_StorageLocation_Filter_Activity.this,"Please Select Atleast One Storage Location");
            }
        }
    }
