...
public class JSA_Controls_Activity extends AppCompatActivity implements View.OnClickListener
{
    ...
    int request_id = 0;
    String hazardcategory_id = "", controls_data = "";
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if (extras != null)
        {
            ...
            controls_data = extras.getString("controls_data");
        }
        ...
        {
            Gson gson = new Gson();
            Type type = new TypeToken<List<JSA_Controls_Activity.Type_Object>>() {}.getType();
            type_list = gson.fromJson(controls_data, type);
            if (type_list.size() > 0)
            {
                title_textview.setText(getResources().getString(R.string.controls)+" ("+type_list.size()+")");
                adapter = new TYPE_ADAPTER(JSA_Controls_Activity.this, type_list);
                list_recycleview.setHasFixedSize(true);
                RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(JSA_Controls_Activity.this);
            ...
            {
                title_textview.setText(getResources().getString(R.string.controls)+" (0)");
    ...
    private class Get_Types extends AsyncTask<Void, Integer, Void>
    {
        ...
        {
            super.onPreExecute();
            custom_progress_dialog.show_progress_dialog(JSA_Controls_Activity.this,getResources().getString(R.string.loading));
            type_list.clear();
        }
        @Override
        protected Void doInBackground(Void... params)
        {
            try
            {
                Cursor cursor = FieldTekPro_db.rawQuery("select * from EtEHSHazctrl where HazardCode = ?", new String[]{hazardcategory_id});
                if (cursor != null && cursor.getCount() > 0)
                {
                    if (cursor.moveToFirst())
                    {
                        do
                        {
                            Type_Object nto = new Type_Object(cursor.getString(2), cursor.getString(5), false);
                            type_list.add(nto);
                        }
    ...
    public class TYPE_ADAPTER extends RecyclerView.Adapter<TYPE_ADAPTER.MyViewHolder>
    {
        ...
        public void onBindViewHolder(final TYPE_ADAPTER.MyViewHolder holder, int position)
        {
            ...
            holder.checkbox.setChecked((notification_type_details_list.get(position).selected_status == true ? true : false));
        }
        @Override
    ...
    public void onClick(View v)
    {
        if (v == back_imageview)
        {
            JSA_Controls_Activity.this.finish();
        }
        ...
        else if (v == submit_button)
        {
            ...
            if(arrayList.size() > 0)
            {
                ...
                intent.putExtra("controls_data",selected_json);
                setResult(RESULT_OK,intent);
                JSA_Controls_Activity.this.finish();
            }
            else
            {
                error_dialog.show_error_dialog(JSA_Controls_Activity.this,"Please Check Atleast One Control");
            }
        }
    }
