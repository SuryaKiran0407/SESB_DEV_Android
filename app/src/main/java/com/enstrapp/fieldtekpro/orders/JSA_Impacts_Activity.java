...
public class JSA_Impacts_Activity extends AppCompatActivity implements View.OnClickListener
{
    ...
    private class Get_Types extends AsyncTask<Void, Integer, Void>
    {
        ...
        {
            super.onPreExecute();
            custom_progress_dialog.show_progress_dialog(JSA_Impacts_Activity.this,getResources().getString(R.string.loading));
            type_list.clear();
        }
        @Override
        protected Void doInBackground(Void... params)
        {
            try
            {
                Cursor cursor = FieldTekPro_db.rawQuery("select * from EtEHSHazimp where HazardCode = ?", new String[]{hazardcategory_id});
                if (cursor != null && cursor.getCount() > 0)
                {
                    if (cursor.moveToFirst())
                    {
                        do
                        {
                            Type_Object nto = new Type_Object(cursor.getString(2), cursor.getString(3), false);
                            type_list.add(nto);
                        }
    ...
    public void onClick(View v)
    {
        ...
        else if (v == submit_button)
        {
            ...
            {
                error_dialog.show_error_dialog(JSA_Impacts_Activity.this,"Please Check Atleast One Impact");
            }
        }
    }
