        ...
        jsa_add_activity.add_fab.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                if(Selected_status)
                {
                    ...
                    description_textview.setText("Do you want to delete selected Impacts & Controls ?");
                ...
                {
                    JSA_Hazards_Fragment jobsteps_fragment = (JSA_Hazards_Fragment)getFragmentManager().findFragmentByTag(makeFragmentName(R.id.viewpager,4));
                    List<JSA_Hazards_Fragment.Hazards_List_Object> list_objects  = jobsteps_fragment.getHazardListData();
                    if (list_objects.size() > 0)
                    {
                        ...
                        Intent intent = new Intent(getActivity(), JSA_Add_ImpactsControls_Activity.class);
                        intent.putExtra("hazards_list",json);
                        startActivityForResult(intent,add_impactscontrols);
                    }
                    else
                    {
                        error_dialog.show_error_dialog(getActivity(),"Please Add Atleast One Job Hazard");
                    }
                }
            }
        });
