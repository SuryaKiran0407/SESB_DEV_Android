...
public class JSA_Change_ImpactsControls_Fragment extends Fragment
{
                ...
                if (cursor != null && cursor.getCount() > 0)
                {
                    if (cursor.moveToFirst())
                    {
                        do
                        {
                            ...
                            try
                            {
                                Cursor cursor1 = FieldTekPro_db.rawQuery("select * from EtJSAImp where RiskID = ?", new String[]{RiskID});
                                if (cursor1 != null && cursor1.getCount() > 0)
                                {
                                    if (cursor1.moveToFirst())
                                    {
                                        do
                                        {
                                            impact_data_StringBuilder.append(cursor1.getString(6));
                            ...
                            try
                            {
                                Cursor cursor1 = FieldTekPro_db.rawQuery("select * from EtJSARskCtrl where RiskID = ?", new String[]{RiskID});
                                if (cursor1 != null && cursor1.getCount() > 0)
                                {
                                    if (cursor1.moveToFirst())
                                    {
                                        do
                                        {
                                            control_data_StringBuilder.append(cursor1.getString(22));
                            ...
                            if(impact_data_StringBuilder.toString().length() > 0 || control_data_StringBuilder.toString().length() > 0)
                            {
                                ImpactsControls_List_Object olo = new ImpactsControls_List_Object(cursor.getString(7), cursor.getString(14), "", impact_data_StringBuilder.toString(), "", control_data_StringBuilder.toString(), "", false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data)
    {
        ...
        if (data != null && !data.equals(""))
        {
            if(requestCode == add_impactscontrols)
            {
                ...
                Type type1 = new TypeToken<List<JSA_Controls_Activity.Type_Object>>() {}.getType();
                ...
                ImpactsControls_List_Object olo = new ImpactsControls_List_Object(hazardtype_id, hazardtype_text, impact_data, stringBuilder.toString(), controls_data, stringBuilder1.toString(), uniqueKey.toString(), false);
                impactscontrols_list.add(olo);
                if(impactscontrols_list.size() > 0)
                {
                    recyclerView.setVisibility(View.VISIBLE);
    ...
    public class Assess_Adapter extends RecyclerView.Adapter<Assess_Adapter.MyViewHolder>
    {
        ...
        {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.jsa_impactscontrols_list_data, parent, false);
    ...
    public class ImpactsControls_List_Object
    {
        ...
        public ImpactsControls_List_Object(String hazardtype_id, String hazardtype_text, String impact_data, String impact_string, String controls_data, String controls_string, String uuid, boolean isSelected)
        {
            this.hazardtype_id = hazardtype_id;
            this.hazardtype_text = hazardtype_text;
            this.impact_data = impact_data;
