...
public class JSA_Add_Activity extends AppCompatActivity implements View.OnClickListener
{
                            ...
                            if (loc_id != null && !loc_id.equals(""))
                            {
                                if(jobstep_list_object.size() > 0)
                                {
                                    if(hazards_list_object.size() > 0)
                                    {
                                        ...
                                        description_textview.setText("Do you want to submit for JSA creation ?");
                                    ...
                                    {
                                        error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Add Atleast One Hazard");
                                    }
                                }
                                else
                                {
                                    error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Add Atleast One Job Step");
                                }
                            }
                            else
                            {
                                error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Select Location Structure");
                            }
                        }
                        else
                        {
                            error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Select Operation Status");
                        }
                    }
                    else
                    {
                        error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Select Job");
                    }
                }
                else
                {
                    error_dialog.show_error_dialog(JSA_Add_Activity.this,"Please Select Title");
                }
            }
            else
            {
    ...
    private class Get_Token extends AsyncTask<Void, Integer, Void>
    {
        ...
        protected void onPostExecute(Void result)
        {
            ...
            {
                custom_progress_dialog.dismiss_progress_dialog();
                error_dialog.show_error_dialog(JSA_Add_Activity.this,"Unable to process JSA Create. Please try again");
            }
        }
    }
    ...
    {
        String jsa_create_status = "Unable to process JSA Create. Please try again.";
        @Override
        protected void onPreExecute()
        {
            super.onPreExecute();
        ...
        protected Void doInBackground(String... params)
        {
            try
            {
                jsa_create_status = JSA_Create.Post_JSA_Data(JSA_Add_Activity.this, title, remark, job_id, job_text, aufnr, assessTeam_list_objects, opstatus_id, opstatus_text, loc_id, loc_text, jobstep_list_object, hazards_list_object, impactsControls_list_objects);
            }
            catch (Exception e)
            {
            }
        ...
        protected void onPostExecute(Void result)
        {
            ...
            if (jsa_create_status!= null && !jsa_create_status.equals(""))
            {
                if(jsa_create_status.startsWith("S"))
                {
                    ...
                    Glide.with(JSA_Add_Activity.this).load(R.drawable.success_checkmark).into(imageview);
                    success_dialog.show();
                    ok_button.setOnClickListener(new View.OnClickListener()
                    {
                        @Override
